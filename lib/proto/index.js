...
$root.OSMPBF = (function() {
    ...
    OSMPBF.Way = (function() {
        ...
        Way.decode = function decode(reader, length) {
            if (!(reader instanceof $Reader))
                reader = $Reader.create(reader);
            const end = length === undefined ? reader.len : reader.pos + length, message = new $root.OSMPBF.Way();
            while (reader.pos < end) {
                ...
                case 8:
                    ...
                    if ((tag & 7) === 2) {
                        const end2 = reader.uint32() + reader.pos;
                        while (reader.pos < end2) {
                            //const si = reader.sint64();

                            //console.log('si', si);
                            //console.log('si.toNumber()', si.toNumber());

                            // toNumber
                            //message.refs.push(si); // a strange data type, does not look so optimal.
                            message.refs.push(BigInt(reader.sint64().toNumber()));
                        }
                    } else {
                        //const si = reader.sint64();
                        //console.log('si', si);
