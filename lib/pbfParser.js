/*
 * OSM PBF (Protocol Buffer Binary Format) File Structure Documentation
 * ===================================================================
 * 
 * This file implements a parser for OpenStreetMap PBF files. The PBF format is a binary
 * representation of OSM data using Google Protocol Buffers for efficient storage and parsing.
 * 
 * PBF File Structure (from outermost to innermost):
 * 
 * 1. FILE LEVEL:
 *    - [1] PBF File contains multiple FileBlocks
 *    - Each FileBlock is a self-contained unit that can be processed independently
 *    - FileBlocks are read sequentially from start to end of file
 * 
 * 2. FILEBLOCK LEVEL:
 *    - [1] BlobHeader (4-byte length prefix + protobuf BlobHeader message)
 *         └── Contains: type ("OSMHeader" or "OSMData"), datasize, indexdata (optional)
 *    - [1] Blob (protobuf Blob message, size specified in BlobHeader.datasize)
 *         └── Contains: raw data OR compressed data (zlib/lzma/bzip2) + raw_size
 * 
 * 3. BLOB CONTENT LEVEL (after decompression):
 *    a) OSMHeader Blob: Contains HeaderBlock with metadata
 *       - bbox: Optional bounding box of the data
 ...
 * 
 *    b) OSMData Blob: Contains PrimitiveBlock with actual OSM data
 *       - stringtable: Array of UTF-8 strings referenced by index throughout the block
 *       - primitivegroup[]: Array of primitive groups containing nodes, ways, relations
 *       - granularity: Coordinate precision (default: 100 nanodegrees)
 *       - lat_offset, lon_offset: Offset values for coordinates
 *       - date_granularity: Timestamp precision (default: 1000 milliseconds)
 * 
 * 4. PRIMITIVEGROUP LEVEL (within PrimitiveBlock):
 *    - Contains exactly ONE of:
 *      a) nodes[]: Individual Node messages (rarely used)
 *      b) dense: DenseNodes message (packed node representation - most common)
 *      c) ways[]: Way messages
 *      d) relations[]: Relation messages
 *      e) changesets[]: Changeset messages (rare)
 * 
 * 5. DENSE NODES FORMAT (most efficient node storage):
 *    - id[]: Delta-encoded node IDs
 *    - lat[], lon[]: Delta-encoded coordinates (in granularity units + offset)
 *    - keys_vals[]: Interleaved key/value string indices, terminated by 0
 *    - denseinfo: Optional metadata (versions, timestamps, changesets, users)
 * 
 * 6. STRING TABLE FORMAT:
 *    - Index 0: Always empty string ("")
 *    - Index 1+: UTF-8 encoded strings referenced throughout the block
 *    - All keys, values, usernames, etc. are stored as indices into this table
 *    - Strings are stored once and referenced by index for efficiency
 * 
 * 7. COORDINATE ENCODING:
 *    - Stored as: (lat_offset + granularity * delta_lat) / 1e9 degrees
 *    - Default granularity: 100 nanodegrees = 1e-7 degrees
 *    - Delta encoding: each coordinate is stored as difference from previous
 * 
 * PARSING STRATEGY:
 * - Read FileBlocks sequentially (can be parallelized)
 * - Decompress Blobs (CPU intensive - good for worker threads)
 * - Parse StringTable first (needed for all other data)
 * - Process PrimitiveGroups based on type (nodes, ways, relations)
 * - Handle delta decoding for coordinates and IDs
 * - Lazy evaluation: decode strings only when accessed
 * 
 * PERFORMANCE CONSIDERATIONS:
 * - Decompression is CPU intensive - use worker threads for large blocks
 * - StringTable access is frequent - cache decoded strings
 * - Delta decoding requires sequential processing within each DenseNodes block
 * - Memory pressure: process blocks incrementally, don't load entire file at once
 * 
 * DATA FLOW:
 * File → FileBlocks → BlobHeaders → Blobs → (decompress) → PrimitiveBlocks → 
 * PrimitiveGroups → (decode with StringTable) → OSM Objects (nodes/ways/relations)
 */
    ...
    readFileBlocks(fd, function(err, fileBlocks){
        ...
        function readBlock(fileBlock, callback){
            return readBlob(fileBlock, function(err, blob){
                ...
                // Enhanced intelligent decompression worker usage based on multiple factors
                const dataSize = blob.zlibData ? blob.zlibData.length : 0;
                const rawSize = blob.rawSize || 1;
                const compressionRatio = dataSize / rawSize;
                
                // More sophisticated thresholds for decompression worker usage
                ...
                // Determine if we should use decompression workers and at what priority
                ...
                // Determine priority for decompression worker scheduling
                let priority = 'normal';
