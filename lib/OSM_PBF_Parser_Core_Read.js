...
class OSM_PBF_Parser extends Evented_Class {
  ...
  async parse() {
    ...
    read_stream.on('end', () => {
      ...
      console.log(`\nCompleted parsing. Total time: ${elapsed.toFixed(1)}s, Total data: ${total_mb}MB, Overall speed: ${overall_mb_s}MB/s`);
      this.raise('end', { elapsed, total_mb, overall_mb_s });
    });
...
if (require.main === module) {
  ...
  const parser = new OSM_PBF_Parser(pbf_path, { verbose: true, highWaterMark });
